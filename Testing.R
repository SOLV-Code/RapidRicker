...
sr.use <- SR_Sample[SR_Sample$Stock == "Stock1",] %>% select(Year, Spn, Rec,logRpS) %>%
                  mutate(Spn = Spn/10^6,Rec = Rec/10^6)
sr.use




max.spn <- max(sr.use$Spn, na.rm = TRUE)
max.spn
...
det.test <- calcDetRickerBM(sr_obj = sr.use %>% mutate(Spn = Spn*10^6,Rec = Rec*10^6), min.obs = 15)

print(paste("Smsy =", round(ricker.test$Medians["Smsy_p"]*10^6)))
print(paste("Smsy Det=", round(det.test["Smsy_p"])))
print(paste("Smax =", round(ricker.test$Medians["Smax"]*10^6)))
print(paste("Smax Det =", round(det.test["Smax"])))
print(paste("Max Obs Spn =", round(max.spn* 10^6)))
print(paste("Mean Obs Spn=", round(mean(sr.use$Spn*10^6, na.rm = TRUE))))
